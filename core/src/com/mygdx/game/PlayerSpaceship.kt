...
class PlayerSpaceship : Spaceship{
    ...
    override var vertices = floatArrayOf(
            Constants.HORIZ_SCALE_RATIO * 0.0f+Constants.SPACESHIP_INITIAL_X_POS,
            Constants.VERT_SCALE_RATIO * 9.0f+Constants.SPACESHIP_INITIAL_Y_POS,
            Constants.HORIZ_SCALE_RATIO * 48.0f+Constants.SPACESHIP_INITIAL_X_POS,
            Constants.VERT_SCALE_RATIO * 75.0f+Constants.SPACESHIP_INITIAL_Y_POS,
            Constants.HORIZ_SCALE_RATIO * 98.0f+Constants.SPACESHIP_INITIAL_X_POS,
            Constants.VERT_SCALE_RATIO * 9.0f+Constants.SPACESHIP_INITIAL_Y_POS
    )
    ...
    override fun moveRight(deltaX: Float){
        if(this.moveRight && (this.x + deltaX + Constants.SPACESHIP_WIDTH <= GameInfo.GAME_WIDTH)) {
    ...
    override fun draw(game: SpaceInvadersGame, texture: Texture) {
        game.getSpriteBatch().draw(texture, this.x, this.y, Constants.SPACESHIP_WIDTH, Constants.SPACESHIP_HEIGHT)
        drawShots(game)
    }

    fun createShot(speedX: Float, speedY: Float){
        val x: Float = Constants.SPACESHIP_WIDTH/2 + this.x - 7.0f * Constants.HORIZ_SCALE_RATIO
    ...
    fun removeOutterShots(){
        ...
        for (shot in shotIterator){
            if(shot.y > GameInfo.GAME_HEIGHT){
                shotIterator.remove()
            }
        }
    }
    ...
    fun accelerometerMove(acc: Float){
        val accScale = 3 * Constants.HORIZ_SCALE_RATIO
        val newPos = this.x + acc * accScale
        if (newPos >= 0 && newPos <= GameInfo.GAME_WIDTH - Constants.SPACESHIP_WIDTH) {
            this.setPosition(this.x + acc * accScale, this.y)
            for (i in 0 until this.vertices.size) {
                if (i % 2 == 0) {
                    this.vertices[i] = this.vertices[i] + (acc * accScale)
                    this.updateSpaceshipPoly()
                }
            }
        }
