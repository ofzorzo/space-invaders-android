...
class EnemyHorde(private val playerSpaceship: PlayerSpaceship, private val gameRunning: GameRunning) {
    ...
    fun moveHorde(){
        ...
        for(i in 0 until this.enemyHorde.size){ // tests if any of the remaining enemy ships hits one of the side walls; if it does, we should change lanes and the movement direction
            val guardSpace = 5.0f * Constants.HORIZ_SCALE_RATIO
            if (this.moveRight && this.enemyHorde[i].x +Constants.ENEMY_WIDTH+(speedDifficCoeff *this.difficulty)> GameInfo.GAME_WIDTH- guardSpace) {
                this.changeLane = true
                this.moveLeft = true
                this.moveRight = false
                break
            }
            else if (this.moveLeft && this.enemyHorde[i].x -(speedDifficCoeff *this.difficulty)< guardSpace){
                this.changeLane = true
                this.moveLeft = false
                this.moveRight = true
                break
        ...
        for(i in 0 until this.enemyHorde.size){
            ...
            enemyHorde[i].setSpeed(speedDifficCoeff *this.difficulty)
